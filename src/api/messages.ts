...
export async function handleSendMessage(c: Context) {
  ...
  try {
    ...
    if (!mailgunRequest.from || !mailgunRequest.to.length || !mailgunRequest.subject) {
      logger.warn("Missing required fields", {
        hasFrom: !!mailgunRequest.from,
        hasTo: mailgunRequest.to.length > 0,
        hasSubject: !!mailgunRequest.subject,
      });
      return c.json({ message: "Missing required fields: from, to, subject" }, 400);
    }
    ...
    if (!registry.has(providerName)) {
      ...
      return c.json({ message: `Provider "${providerName}" not configured` }, 500);
    }
    ...
    if (failed.length === results.length) {
      logger.error("All emails failed to send", { errors: failed.map((f) => f.error) });
      return c.json({ message: "Failed to send emails" }, 500);
    }

    if (failed.length > 0) {
      logger.warn("Some emails failed", { failed: failed.length, total: results.length });
    }
    ...
    logger.info("Message queued", { id: response.id, emailCount: results.length });
