...
import { processBatchesParallel } from '../src/utils/batch'
...
describe('Performance Tests', () => {
  describe('Batch Processing', () => {
    ...
    test('handles 1000 emails efficiently', async () => {
      ...
      console.log(
        `1000 emails processed in ${duration}ms (${Math.round((1000 / duration) * 1000)} emails/sec)`,
      )
    })
...
describe('Throughput Benchmarks', () => {
  test('measure maximum theoretical throughput', async () => {
    ...
    const results: { count: number; duration: number; perSec: number }[] = []
